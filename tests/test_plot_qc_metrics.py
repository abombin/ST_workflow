...
from quality_control import plot_qc_metrics
...
class TestPlotQCMetrics(unittest.TestCase):
    ...
    def test_plot_qc_metrics_with_group_column(self):
        adata = self.create_test_adata()
        result = plot_qc_metrics(adata, group_column="group")
    ...
    def test_plot_qc_metrics_with_rotation(self):
        adata = self.create_test_adata()
        result = plot_qc_metrics(adata, rotation=45)
        self.assertIsInstance(result["figure"], Figure)
